...
function writeKotlinSources(srcDir: string, pkgName: string, config: BuildConfig) {
  ...
  data class CentreLoginResponse(val allowed: Boolean, val message: String)
    ...
    data class OperatorRegResponse(val success: Boolean, val operator: OperatorData?, val message: String?, val alreadyRegistered: Boolean)
    data class OperatorData(val id: Int, val name: String, val phone: String, val aadhaar: String, val centreCode: String?, val examId: Int?, val examName: String?)
